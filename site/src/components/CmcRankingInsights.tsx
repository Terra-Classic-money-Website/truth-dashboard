...
import type { CmcRankingView } from "../data/selectors/cmcRanking";

type CmcRankingInsightsProps = {
  insights: CmcRankingView["insights"];
};
...
export default function CmcRankingInsights({
  insights,
}: CmcRankingInsightsProps) {
  return (
    <section className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold text-white">Insights &amp; Health</h2>
        <p className="mt-1 text-sm text-slate-400">
          Derived from archived CoinMarketCap coin pages and the latest static CMC rank observation. Lower rank is better.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Median, best, and worst observed CMC rank by calendar month.
          </p>
        ...
        <Card>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
              <p className="text-xs uppercase tracking-wider text-slate-500">Timemap captures</p>
              <p className="mt-2 text-lg font-semibold text-white">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">Fetch or rank parse unavailable</p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
      ...
      <Card>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Exact source observations behind the latest plotted points. The rank source identifies the CMC page field that was parsed.
            </p>
          </div>
          ...
          <table className="w-full min-w-[800px] text-left text-sm">
            <thead className="bg-slate-950/60 text-xs uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className="whitespace-nowrap px-4 py-3">CMC rank</th>
                <th className="whitespace-nowrap px-4 py-3">CMC label</th>
                <th className="whitespace-nowrap px-4 py-3">Parsed field</th>
      ...
      <Card>
        ...
        <div className="mt-5 rounded-xl border border-sky-300/20 bg-sky-300/5 p-4">
          <p className="text-xs uppercase tracking-wider text-sky-200">Latest rank</p>
          <p className="mt-2 text-lg font-semibold text-white">
        ...
        <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm">
          ...
          <a
            ...
            className="text-slate-300 underline decoration-slate-600 underline-offset-2 hover:text-amber-200"
          >
            Open Wayback Timemap
          </a>
          <a
            href={insights.currentEvidence.sourceUrl}
            target="_blank"
            rel="noreferrer"
            className="text-slate-300 underline decoration-slate-600 underline-offset-2 hover:text-amber-200"
          >
            Open current CMC source
          </a>
        </div>
      </Card>
    </section>

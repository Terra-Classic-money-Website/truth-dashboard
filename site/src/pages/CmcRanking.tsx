...
import CmcRankingInsights from "../components/CmcRankingInsights";
...
import { selectCmcRanking, type CmcRankingMode } from "../data/selectors";
...
export default function CmcRanking() {
  const { data: snapshot, error } = getSnapshot("cmc-ranking");
  const [windowId, setWindowId] = useState("all");
  const [mode, setMode] = useState<CmcRankingMode>("snapshots");
  const chartWrapRef = useRef<HTMLDivElement>(null);
  const [chartHeight, setChartHeight] = useState<number | null>(null);
  const viewportWidth = useViewportWidth();
  const isMobile = viewportWidth < 640;
  const view = useMemo(
    () => (snapshot ? selectCmcRanking(snapshot, windowId, mode) : null),
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="Terra Classic Market Position"
        ...
        <Card className="p-0">
          ...
          >
            <TimeSeriesChart
              ...
              invertY
              showPoints
              maxGapDays={mode === "monthly" ? 180 : 120}
            />
          </div>
        </Card>
      </section>

      <CmcRankingInsights insights={view.insights} />
    </div>
  );
}

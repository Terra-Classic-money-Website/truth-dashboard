...
function clampNumber(value: number | null) {
...
const balancesWeekly: BalancePoint[] = balanceRows
  .map((row) => ({
...
const balancesByDate = new Map<string, BalancePoint>();
balancesWeekly.forEach((row) => balancesByDate.set(row.t, row));

const groups = new Map<string, MarkerGroup>();
const ensureGroup = (markerTime: string, dropTime: string | null) => {
